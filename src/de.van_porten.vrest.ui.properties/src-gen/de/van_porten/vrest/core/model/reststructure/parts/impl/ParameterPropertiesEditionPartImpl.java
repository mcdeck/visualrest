...
public class ParameterPropertiesEditionPartImpl extends CompositePropertiesEditionPart implements ISWTPropertiesEditionPart, ParameterPropertiesEditionPart {
		...
		composer = new PartComposer(parameterStep) {
			...
			public Composite addToPart(Composite parent, Object key) {
				...
				if (key == ReststructureViewsRepository.Parameter.Properties.paramType) {
					return createParamTypeFlatComboViewer(parent);
				}
				return parent;
			}
		};
	...
	protected Composite createPropertiesGroup(Composite parent) {
		...
		propertiesGroup.setText(ReststructureMessages.ParameterPropertiesEditionPart_PropertiesGroupLabel);
	 ...
	 * 
	 */
	protected Composite createParamTypeFlatComboViewer(Composite parent) {
		SWTUtils.createPartLabel(parent, ReststructureMessages.ParameterPropertiesEditionPart_ParamTypeLabel, propertiesEditionComponent.isRequired(ReststructureViewsRepository.Parameter.Properties.paramType, ReststructureViewsRepository.SWT_KIND));
		paramType = new EObjectFlatComboViewer(parent, !propertiesEditionComponent.isRequired(ReststructureViewsRepository.Parameter.Properties.paramType, ReststructureViewsRepository.SWT_KIND));
		...
		paramType.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ParameterPropertiesEditionPartImpl.this, ReststructureViewsRepository.Parameter.Properties.paramType, PropertiesEditionEvent.CHANGE, PropertiesEditionEvent.SET, null, getParamType()));
			}
		...
		SWTUtils.createHelpButton(parent, propertiesEditionComponent.getHelpContent(ReststructureViewsRepository.Parameter.Properties.paramType, ReststructureViewsRepository.SWT_KIND), null); //$NON-NLS-1$
		return parent;
	}
